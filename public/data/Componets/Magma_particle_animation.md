...
<html lang="en">
  ...
  <head>
    <meta charset="UTF-8">
    <title>Magma Particle Field</title>
    ...
    * {
    ...
    .glow {
        ...
        background: radial-gradient(circle at 50% 50%,
            rgba(255, 60, 0, 0.05) 0%,
            rgba(255, 0, 40, 0.08) 40%,
            transparent 75%
        );
...
<div class="glow"></div>
<div class="ui-hint">Click center for shockwave</div>
...
function generateShape(t, angle, phase) {
    ...
    let z = radius * Math.sin(t * 8 + phase) * 0.8;
    
    return { x, y, z };
}

const particleCount = 130000;
    ...
    const material = new THREE.ShaderMaterial({
        ...
        vertexShader: `
            ...
            void main() {
                ...
                vec3 animatedPos = originalPos + normOrig * displacement * 6.0;

                vec3 pos = animatedPos;
                
                float waveAge = time - shockwaveTime;
                ...
                if (waveAge > 0.0 && waveAge < 3.0) {
                    ...
                    if (abs(diff) < waveWidth) {
                        ...
                        float zForce = rippleShape * 20.0 * decay;
                        pos.z += zForce;
                        
                        pos.xy += normalize(pos.xy) * rippleShape * 5.0 * decay;
                        
                        vColor += vec3(0.5, 0.4, 0.2) * rippleShape * decay;
                    }
                }
                ...
                if (influence > 0.001) {
                    ...
                    float dispMag = velInfluence * (5.0 + randomFactor * 6.0);
                    pos += dir * dispMag;
                ...
                gl_PointSize = size * sizeFactor * perspective * pixelRatio;
            }
        `,
        fragmentShader: `
            uniform float time;
...
function init() {
    ...
    document.addEventListener('click', () => {
        if(particles) {
            particles.material.uniforms.shockwaveTime.value = time;
        }
    });
...
function onWindowResize() {
    ...
    renderer.setSize(window.innerWidth, window.innerHeight);
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5));
    if(particles) particles.material.uniforms.pixelRatio.value = renderer.getPixelRatio();
}
