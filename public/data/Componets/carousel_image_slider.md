---
title: "Interactive 3D Carousel Using HTML, CSS & JavaScript | Drag & Scroll Animation Effect"
...
## 🧠 Description
This project showcases a stunning 3D Carousel built using HTML, CSS, and Vanilla JavaScript that supports drag and scroll-wheel interactions.
Each card dynamically adjusts its position, depth, and rotation based on user input, creating a smooth parallax-style motion.
The design includes custom cursors, glowing gradient backgrounds, and responsive card layouts for a premium, modern visual experience.
...
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <title>Carousel with drag and wheel - CodewithLord</title>
    <link
      rel="stylesheet"
  ...
  <body>
    ...
    <a href="https://portfolio-ochre-rho-ax0gh5qo21.vercel.app/contact" target="_blank" class="logo">C</a>
    <div class="social">
      
      <a href="https://www.instagram.com/code_with_lord/" target="_blank"
        ><svg>
...
The HTML sets up the structure of the carousel with multiple .carousel-item elements, each containing an image, title, and number.

The .carousel acts as the main container, and each .carousel-item holds a .carousel-box for inner layout control.

A .layout div is added for decorative vertical line and branding text.
...
The .carousel-item elements are positioned absolutely and use CSS custom properties (--x, --y, --rot, --opacity) to control dynamic transformations.

Each card smoothly transitions using cubic-bezier animations to give natural motion when dragging or scrolling.

The .carousel-box adds a dark gradient overlay and shows the title and number with subtle shadows.

The .layout and .box elements provide a minimal aesthetic grid line and branding text on the left side.

The .logo is circular and placed at the top-right corner as a clickable link to your portfolio.

The .cursor and .cursor2 create custom animated cursors, following mouse movement with a smooth transition.
...
Constants speedWheel and speedDrag control the sensitivity for mouse wheel and drag actions.

The getZindex() function calculates dynamic depth (z-index) for each carousel item based on its active position.

displayItems() updates each item’s CSS variables (--zIndex, --active) so that cards animate to the correct 3D position.

The animate() function ensures the carousel movement stays within bounds (0–100%) and updates positions in real time.

Event listeners (mousewheel, mousedown, mousemove, mouseup, and touch equivalents) allow both desktop and mobile interaction.

Clicking a carousel item directly sets it as active, triggering a centered animation.

Cursor elements are animated with JS to follow mouse movements smoothly, enhancing user interactivity.